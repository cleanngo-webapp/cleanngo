...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1" id="table-description">
                        @if($status === 'cancelled')
                            View all your cancelled jobs and their details
                        @else
        ...
        <div id="completed-jobs-table-container" class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody id="completed-jobs-table-body" class="bg-white divide-y divide-gray-200">
                    @foreach($bookings as $b)
                    <tr class="hover:bg-gray-50 transition-colors completed-job-card" data-booking-id="{{ $b->id }}">
    ...
    @include('components.booking-info-modal', [
        'modalId' => 'completed-job-info-modal'
    ])
    ...
    <script>
    // Search and Sort functionality for completed jobs
    let currentSort = '{{ $sort ?? "scheduled_start" }}';
    let currentSortOrder = '{{ $sortOrder ?? "desc" }}';
    let searchTimeout;

    // Search functionality with AJAX
    document.getElementById('search-completed-jobs').addEventListener('input', function() {
    ...
    // AJAX search function for completed jobs
    function performCompletedSearch() {
        const searchTerm = document.getElementById('search-completed-jobs').value;
        const statusFilter = document.getElementById('status-filter').value;
        const url = new URL('{{ route("employee.completed-jobs") }}', window.location.origin);
        ...
        fetch(url)
            ...
            .then(html => {
                ...
                const newTableBody = doc.getElementById('completed-jobs-table-body');
    ...
    function clearFilters() {
        // Clear search input
        const searchInput = document.getElementById('search-completed-jobs');
        if (searchInput) {
            searchInput.value = '';
        }
    ...
    // Payment proof modal handlers for completed jobs
    function openCompletedPaymentProof(proofId) {
        ...
        fetch(`/employee/payment-proof/${proofId}/details`)
